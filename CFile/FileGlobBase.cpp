...
void* opendirectory(char *filename);
void closedirectory(void* dir);
char* readdirectory(void* dir, int* type);
...
DoRecursion:
    ...
    // Start the find.
    int type;
    char* pFoundFileName = NULL;
    char foundFileName[1024];
    void* handle = opendirectory(basePath);
    ...
    *basePathEndPtr = 0;
    ...
    if (handle)
    {
        while(pFoundFileName = readdirectory(handle, &type))
        {
            strcpy(foundFileName, pFoundFileName);
            sprintf(tmpPath, "%s/%s", rootPath, pFoundFileName);

            // Is the file a directory?
            if ( (type == IS_DIR)  &&  !matchFiles )
            {
                // Do a wildcard match.
                if ( WildMatch( matchPattern, foundFileName, false ) )
                {
                    // It matched.  Let's see if the file should be ignored.
                    bool ignore = false;

                    // Knock out "." or ".." if they haven't already been.
                    size_t len = strlen( foundFileName );
                    foundFileName[ len ] = '/';
                    foundFileName[ len + 1 ] = '\0';

                    // See if this is a directory to ignore.
                    ignore = MatchIgnorePattern( foundFileName );
                    ...
                    if ( !ignore )
                    {
                        ...
                        fileList.push_back( foundFileName );
                    }
                }
            }
            else if ( (type == IS_FILE)   &&  matchFiles )
            {
                // Do a wildcard match.
                if ( WildMatch( matchPattern, foundFileName, false ) )
                {
                    // It matched.  Let's see if the file should be ignored.
                    bool ignore = MatchIgnorePattern( foundFileName );

                    // Is this pattern exclusive?
                    if ( !ignore  &&  m_exclusiveFilePatterns.begin() != m_exclusiveFilePatterns.end() )
                    {
                        ignore = !MatchExclusivePattern( foundFileName );
                    }
...
void* opendirectory(char *filename)
{
    ...
    if ((pattern == NULL) || (dir == NULL)) return NULL;
...
char* readdirectory(void* dir, int* type)
{
    for (;;)
    {
        if (!((directory_type*)dir)->first)
        {
            if (!FindNextFileA(((directory_type*)dir)->handle, &(((directory_type*)dir)->data)))
                return NULL;
        }
        else
        {
            ((directory_type*)dir)->first = FALSE;
        }
        if (strcmp(((directory_type*)dir)->data.cFileName, ".") != 0 && strcmp(((directory_type*)dir)->data.cFileName, "..") != 0)
        {
            if(type) (*type) = (((directory_type*)dir)->data.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) ? IS_DIR: IS_FILE;
            return ((directory_type*)dir)->data.cFileName;
        }
    }
    return NULL;
}
...
void closedirectory(void* dir)
{
    FindClose(((directory_type*)dir)->handle);
    free(dir);
}
...
char * readdirectory(void* dir, int* type)
{
    for (;;)
    {
        struct dirent *dent = readdir(((directory_type*)dir)->handle);
        if(dent == NULL)
            return NULL;
        if(strcmp(dent->d_name, ".") != 0 && strcmp(dent->d_name, "..") != 0)
        {
            char tmpPath[1024];
            sprintf(tmpPath, "%s/%s", ((directory_type*)dir)->path, dent->d_name);
            if(type)
            {
                struct stat statbuf;
                if(followLinks)
...
void closedirectory(void* dir)
{
    closedir(((directory_type*)dir)->handle);
    free(dir);
}
