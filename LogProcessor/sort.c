...
#define error(rc, err, str) fprintf(stderr, "\n%s, errno(%d)\n\n", str, err); exit(rc)
...
tempname (void)
{
    ...
    int len = (int)strlen (temp_dir);
    char *name = (char*)malloc (len + 1 + sizeof ("sort") - 1 + 5 + 5 + 1);
    ...
    node = (struct tempnode *) malloc (sizeof (struct tempnode));
    node->name = name;
...
fillbuf (struct buffer *buf, FILE *fp)
{
        ...
        || !memchr (buf->buf, eolchar, buf->used)))
    {
        if (buf->used == buf->alloc - 1)
        {
            ...
            buf->buf = (char*)realloc (buf->buf, buf->alloc);
        }
        cc = (int)fread (buf->buf + buf->used, 1, buf->alloc - 1 - buf->used, fp);
        if (ferror (fp))
        {
...
{
    lines->alloc = alloc;
    lines->lines = (struct line *) malloc (lines->alloc * sizeof (struct line));
    lines->used = 0;
    lines->limit = limit;
}
...
findlines (struct buffer *buf, struct lines *lines)
{
    ...
    while (beg < lim && (ptr = (char*)memchr (beg, eolchar, lim - beg))
        && lines->used < lines->limit)
    {
        if (lines->used == lines->alloc)
        {
            ...
            lines->lines = (struct line *)
                realloc ((char *) lines->lines,
                lines->alloc * (int)sizeof (struct line));
        }
...
checkfp (FILE *fp, const char *file_name)
{
    ...
    while (1)
    {
        ...
        if (prev_line->length + 1 > alloc)
        {
            ...
            temp.text = (char*)realloc (temp.text, alloc);
        }
...
finish:
    ...
    {
        fprintf (stderr, ("%s: %s:%d: disorder: "), program_name, file_name,
...
mergefps (FILE **fps, register int nfps, FILE *ofp, const char *output_file)
{
    ...
    if (unique)
    {
        ...
        saved.text = (char*)malloc (savealloc);
    }
            ...
            if (!savedflag)
            {
                if (savealloc < lines[ord[0]].lines[cur[ord[0]]].length + 1)
                {
                    while (savealloc < lines[ord[0]].lines[cur[ord[0]]].length + 1)
                        savealloc *= 2;
                    saved.text = (char*)realloc (saved.text, savealloc);
                }
...
sort (char **files, int nfiles, FILE *ofp, const char *output_file)
{
    ...
    ntmp = lines.alloc;
    tmp = (struct line *) malloc (ntmp * sizeof (struct line));
        ...
        while (fillbuf (&buf, fp))
        {
            ...
            if (lines.used > ntmp)
            {
                while (lines.used > ntmp)
                    ntmp *= 2;
                tmp = (struct line *)
                    realloc ((char *) tmp, ntmp * sizeof (struct line));
            }
    ...
    if (n_temp_files)
    {
        tempfiles = (char **) malloc (n_temp_files * sizeof (char *));
        i = n_temp_files;
...
{
    error (SORT_FAILURE, 0, ("invalid field specification `%s'"));
}
...
int SortMain (int argc, char **argv)
{
    ...
    char *minus = "-", *outfile = minus, **files;
    FILE *ofp;
#ifdef SA_INTERRUPT
    ...
    gkey.ignore = NULL;
    gkey.translate = NULL;
    gkey.numeric = gkey.general_numeric = gkey.reverse = 0;
    ...
    for (i = 1; i < argc; ++i)
    {
        ...
        else if (argv[i][0] == '-' && argv[i][1])
        {
            ...
            if (ISDIGIT (*s) || (*s == '.' && ISDIGIT (s[1])))
            {
                ...
                {
                    /* Provoke with `sort -9'.  */
                    error (0, 0, ("when using the old-style +POS and -POS \
                ...
                while (*s)
                {
                    ...
                    case 'k':
                        ...
                        if (t == 0)
                        {
                            ...
                            error (0, 0, ("the starting field number argument \
                        ...
                        if (*s == '.')
                        {
                            if (!ISDIGIT (s[1]))
                            {
                                ...
                                error (0, 0, ("starting field spec has `.' but \
                            ...
                            if (t2 == 0)
                            {
                                ...
                                error (0, 0, ("starting field character offset \
                        ...
                        else if (*s == ',')
                        {
                            ...
                            {
                                /* Provoke with `sort -k1,' */
                                error (0, 0, ("field specification has `,' but \
                            ...
                            {
                                /* Provoke with `sort -k1,0' */
                                error (0, 0, ("ending field number argument \
                                              to the `-k' option must be positive"));
                                badfieldspec (argv[i]);
                            }
                            --t;
                            ...
                            if (*s == '.')
                            {
                                if (!ISDIGIT (s[1]))
                                {
                                    ...
                                    error (0, 0, ("ending field spec has `.' \
...
outer:;
    ...
    if (fflush (ofp) < 0)
        error (SORT_FAILURE, errno, ("%s: write error"));

    if (have_read_stdin && fclose (stdin) == EOF)
        error (SORT_FAILURE, errno, "%s");
