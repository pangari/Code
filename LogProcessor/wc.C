    ...
    */
    ...
    do {
        ...
        for (;;) {
            if (p1 >= p2) {
                p1 = b;
                c = (unsigned int)fread(p1, 1, BUFSIZ, fptr);
                if (c == 0) {
                    if (feof(fptr))
                        break;
                    /*
                    ...
                    */
                    if (ferror(fptr)) {
                        (void) fprintf(stderr, "wc: cannot read %s: ",
                            argv[i]);
                        perror("");
                        status = 2;
                        errflag = 1;
            ...
            if (ISASCII(c)) {
                ...
                if ((len = (p2 - p1)) <
                    (int)MB_CUR_MAX) {
                        ...
                        p1 = b;
                        p2 = p1 + n;
                        c = (unsigned int)fread(p2, 1, BUFSIZ - n, fptr);
                        if ((int)c > 0) {
                            charct += c;
                            p2 += c;
                        }
...
usage()
{
    (void) fprintf(stderr, "usage: wc [-c | -m | -C] [-lw] [file ...]\n");
    exit(2);
}
